...
class UserProfileUpdateApiDocsTest {
	...
	void updateNickname() throws Exception {
		...
		when(userNicknameUpdateService.update(any(CurrentUser.class), any(UserNicknameUpdateRequest.class)))
			.thenReturn(new UserNicknameUpdateResponse(
				1003L,
				"user@example.com",
				"새로운닉네임",
				UserStatus.ACTIVE,
		...
		mockMvc.perform(patch("/v1/users/me/nickname")
			...
			.andDo(document("users-nickname",
				...
				requestFields(
					fieldWithPath("nickname").type(JsonFieldType.STRING).description("변경할 닉네임입니다. 최대 100자까지 허용됩니다.")
				),
				responseFields(
					fieldWithPath("success").type(JsonFieldType.BOOLEAN).description("요청 처리 성공 여부입니다."),
					fieldWithPath("data").type(JsonFieldType.OBJECT).description("닉네임 변경 후 현재 사용자 정보입니다."),
