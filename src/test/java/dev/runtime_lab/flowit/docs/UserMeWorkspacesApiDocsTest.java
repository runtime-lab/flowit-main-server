...
class UserMeWorkspacesApiDocsTest {
	...
	@Test
	void meWorkspaces() throws Exception {
		List<UserMeWorkspaceResponse> response = List.of(
			new UserMeWorkspaceResponse(2001L, "Flowit", "Team workspace", 3L, WorkspaceMemberRole.OWNER, 1779889000L),
			new UserMeWorkspaceResponse(2002L, "Runtime Lab", null, 2L, WorkspaceMemberRole.MEMBER, 1779890000L)
		);
		...
		mockMvc.perform(get("/v1/users/me/workspaces")
			...
			.andDo(document("users-me-workspaces",
				...
				responseFields(
					...
					fieldWithPath("data").type(JsonFieldType.OBJECT).description("현재 사용자가 가입한 워크스페이스 목록 데이터입니다."),
					fieldWithPath("data.items").type(JsonFieldType.ARRAY).description("현재 사용자가 가입한 워크스페이스 목록입니다."),
					fieldWithPath("data.items[].id").type(JsonFieldType.NUMBER).description("워크스페이스 식별자입니다."),
